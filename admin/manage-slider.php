...
if (strlen($_SESSION['adminlogin']) == 0) {
    ...
    if (isset($_GET['del'])) {
        $id = intval($_GET['id']);
        mysqli_query($con, "DELETE FROM tbl_slider WHERE slider_id = '$id'");
    }
}
?>
<!DOCTYPE html>
        ...
        <div class="row">
            ...
            <section class="users_container col-md-10">
                <div class="users_head mb20">
                    <p class="text-capitalize fs16 fw-bolder px10-py20 w100pt bg-default m-0">View Slider</p>
                </div>
                                ...
                                <table class="w100pt">
                                    <thead class="manage-products_head">
                                        <tr
                                            ...
                                            <td class="col-md-4 border-end border-dark">Photo</td>
                                            <td class="col-md-3 border-end border-dark">Page</td>
                                            <td class="col-md-3 border-end border-dark">Date</td>
                                            <td class="col-md-1">Action</td>
                                        </tr>
                                    </thead>
                                    <tbody class="manage-products_body">
                                        <?php
                                        $query = mysqli_query($con, "SELECT tbl_page_slider.*, tbl_slider.* FROM tbl_slider join tbl_page_slider on tbl_page_slider.page_id = tbl_slider.slider_on_page");
                                        $count = 1;
                                        while ($row = mysqli_fetch_array($query)) {
                                            ...
                                            <tr class="odd text-start fs14 w100pt border border-dark">
                                                ...
                                                <td class="col-md-4 p10 border-end border-dark">
                                                    <img src="<?php echo htmlentities($row['slider_img']); ?>" alt="">
                                                </td>
                                                ...
                                                <td class="col-md-1 p10">
                                                    <p class="w100pt m-0">
                                                        <a
                                                            href="edit-slider.php?id=<?php echo htmlentities($row['slider_id']) ?>">
                                                            <i class="fa-solid fa-pen-to-square hover-color-orange"></i>
                                                        </a>
                                                        <a href="manage-slider.php?id=<?php echo htmlentities($row['slider_id']) ?>&del=delete"
                                        ...
                                        $query = mysqli_query($con, "SELECT tbl_page_slider.*, tbl_slider.* FROM tbl_slider join tbl_page_slider on tbl_page_slider.page_id = tbl_slider.slider_on_page");
                                        while ($row = mysqli_fetch_array($query)) {
                                            echo htmlentities($row['slider_id']);
                                        }
                                        ?> -->
                                    </tbody>
                                </table>
