...
if (strlen($_SESSION['adminlogin']) == 0) {
    ...
    if (isset($_POST['submit'])) {
        ...
        $desc = $_POST['desc'];
        $id = intval($_GET['id']);
        $query = mysqli_query($con, "UPDATE tbl_category_post SET category_post_name = '$category', category_post_desc = '$desc'  WHERE category_post_id = '$id'");
                ...
                <section class="category-container mb106">
                    ...
                    <div class="category_body">
                        <form action="" method="post">
                            <?php
                            $id = intval($_GET['id']);
                            $query = mysqli_query($con, "SELECT * FROM tbl_category_post WHERE category_post_id = '$id'");
                            while ($row = mysqli_fetch_array($query)) {
                                ...
                                <div class="control-group mb20 row">
                                    ...
                                    <input type="text" name="category" class="col-sm-9 w400 col-md-9 "
                                        value="<?php echo htmlentities($row['category_post_name']) ?>">
                                </div>
                                <div class="control-group mb20 row">
                                    <label class="col-md-3 col-sm-3 text-end" for="">Description</label>
                                    <textarea name="desc" id="" cols="" rows="5"
                                        class="col-sm-9 w400 col-md-9"><?php echo htmlentities($row['category_post_desc']) ?></textarea>
                                </div>
                                <?php
                            }
                            ?>
